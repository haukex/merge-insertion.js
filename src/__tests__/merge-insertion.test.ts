 ...
 */
import mergeInsertionSort, { _binInsert, _cachedComparator, _groupSizes, _makeGroups, Comparator,
...
test('fisherYates', () => {
  ...
  fisherYates(b, function* () { yield* [5, 1, 5, 0, 2, 2, 0]; throw new Error('no more values') }())
  expect(b).toStrictEqual(['G','E','D','C','A','H','B','F'])
  const c = ['A','B','C']
  fisherYates(c)
  expect(c).not.toStrictEqual(['A','B','C'])
})
